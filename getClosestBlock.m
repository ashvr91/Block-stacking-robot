%This function detects the blocks and returns the X,Y pixel values and Z depth of
%the block with the maximum X value. 


function BlockCoordinates = getClosestBlock()
redThresh = 0.04; % Threshold for red detection
vidDevice = imaq.VideoDevice('winvideo', 2, 'RGB24_640x480',... % Acquire input video stream
                    'ROI', [1 1 640 480], ...
...
release(vidDevice);
depth = 660;
BlockCoordinates = [centroid(end,1),centroid(end,2),depth];
